...
export const flagCoinCollection =
  (collectedCoinid: string): AppThunk =>
          ...
          .then((coins) => {
            ...
            if (collectedCoin) {
              ...
              dispatch(
                updateFlaggedCoins([
                  ...flaggedCoins,
                  { coinid: collectedCoin.coinid, collectOnBlock: chainHeight },
                ])
              );
          ...
          .then((coins) => {
            ...
            coins.forEach((c) => {
              ...
              if (
                c.status === "PENDING" &&
                c.collectedOnBlock &&
                new Decimal(chainHeight)
                  .minus(new Decimal(c.collectedOnBlock))
                  .greaterThan(1)
              ) {
                ...
                dispatch(
                  updateFlaggedCoins(
                    flaggedCoins.filter((coin) => coin.coinid !== c.coinid)
                  )
                );
                dispatch(callAndStoreCoins());
              }
              ...
              const exists = flaggedCoins.filter(
                (coin) => coin.coinid === c.coinid
              );
...
export const { updateFutureBalance, updatePendingStatus, updateFlaggedCoins } =
  coinSlice.actions;
...
export const coinMiddleware = (store: any) => (next: any) => (action: any) => {
  if (
    ...
    selectPendingCoinCollection(store.getState())
  ) {
